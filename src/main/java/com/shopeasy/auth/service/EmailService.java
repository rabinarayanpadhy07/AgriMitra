...
public class EmailService {
    ...
    public void sendOtpEmail(String recipientEmail, String recipientName, String otp) {
        ...
        if (senderEmail == null || senderEmail.trim().isEmpty() || senderEmail.contains("your_email") || senderEmail.equals("${MAIL_USERNAME:}")) {
            ...
            throw new ApiException("Email service is currently not configured on the server. Please set MAIL_USERNAME and MAIL_PASSWORD.");
        }
        ...
        try {
            ...
            helper.setFrom(senderEmail, "AgriMitra");

            mailSender.send(mimeMessage);
            logger.info("OTP email successfully dispatched via Gmail SMTP to {}", recipientEmail);
            ...
            if (msg != null && (msg.contains("SocketTimeoutException") || msg.contains("Connect timed out") || msg.contains("timeout"))) {
                throw new ApiException("Connection timed out reaching SMTP server. Note: Render Free Tier blocks outbound SMTP ports 25, 465, and 587.");
            }
